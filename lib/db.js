...
export async function getProducts({ page = 1, limit = 12, category, brand, search, sort = 'sort_order', featured } = {}) {
  ...
  if (category) {
    where.push('(category = ? OR category LIKE ? OR ? LIKE "%" || category || "%")');
...
export async function getCategories() {
  ...
  const result = await db.prepare(
    "SELECT DISTINCT category FROM products WHERE is_active = 1 AND category IS NOT NULL AND category != '' ORDER BY category ASC"
  ).all();
...
export async function getBrands() {
  ...
  return (result.results || []).map(r => r.brand);
}
